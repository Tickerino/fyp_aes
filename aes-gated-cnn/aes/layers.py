...
class GateVector(_Merge):
    def __init__(self, **kwargs):
        self.supports_masking = True
        super(GateVector, self).__init__(**kwargs)

    def build(self, input_shape):
        input_shape = input_shape
        self.v = K.variable(
            (np.random.rand(
                50, ) * 0.2 - 0.1).astype('float32'), name='v')
        self.b = K.variable(value=0, dtype='float32', name='b')
...
class GateMatrix(_Merge):
    def __init__(self, bias=0, **kwargs):
        self.supports_masking = True
        self.bias = bias
        super(GateMatrix, self).__init__(**kwargs)

    def build(self, input_shape):
        input_shape = input_shape,
        self.v = K.variable(
            (np.random.rand(50, 50) * 0.2 -
             0.1).astype('float32'),
            name='v')
