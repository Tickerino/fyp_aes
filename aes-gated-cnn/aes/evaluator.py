import logging
import numpy as np
from aes.kappa_calculator import quadratic_weighted_kappa as qwk
from aes.kappa_calculator import liaesr_weighted_kappa as lwk
...
class Evaluator():
    def __init__(self, model_type, batch_size, dataset, prompt_id, out_dir,
                 dev_x, test_x, dev_y, test_y, dev_y_org, test_y_org):
        self.model_type = model_type
        self.dataset = dataset
        self.prompt_id = prompt_id
        self.out_dir = out_dir
        if self.model_type in ['gate-positional', 'gate-matrix', 'gate-vector',
                               'concat']:
            self.dev_char_x, self.dev_x = dev_x[0], dev_x[1]
            self.test_char_x, self.test_x = test_x[0], test_x[1]
        else:
    ...
    def dump_ref_scores(self):
        np.savetxt(
            self.out_dir + '/preds/dev_ref.txt', self.dev_y_org, fmt='%i')
        np.savetxt(
            self.out_dir + '/preds/test_ref.txt', self.test_y_org, fmt='%i')

    def dump_predictions(self, dev_pred, test_pred, epoch):
        np.savetxt(
            self.out_dir + '/preds/dev_pred_' + str(epoch) + '.txt',
            dev_pred,
            fmt='%.8f')
        np.savetxt(
            self.out_dir + '/preds/test_pred_' + str(epoch) + '.txt',
            test_pred,
            fmt='%.8f')
    ...
    def evaluate(self, model, epoch, print_info=False):
        if self.model_type in ['gate-positional', 'gate-matrix', 'gate-vector',
            ...
            self.dev_loss, self.dev_metric = model.evaluate(
                [self.dev_char_x, self.dev_x],
                self.dev_y,
                batch_size=self.batch_size,
                verbose=0)
            self.test_loss, self.test_metric = \
                model.evaluate(
                    [self.test_char_x, self.test_x],
                    self.test_y,
                    batch_size=self.batch_size,
                    verbose=0)
            ...
            self.dev_loss, self.dev_metric = model.evaluate(
                self.dev_x, self.dev_y, batch_size=self.batch_size, verbose=0)
            self.test_loss, self.test_metric = model.evaluate(
                ...
                verbose=0)
            self.dev_pred = model.predict(
                self.dev_x, batch_size=self.batch_size).squeeze()
            self.test_pred = model.predict(
                self.test_x, batch_size=self.batch_size).squeeze()
        self.dev_pred = self.dataset.convert_to_dataset_friendly_scores(
            self.dev_pred, self.prompt_id)
        self.test_pred = self.dataset.convert_to_dataset_friendly_scores(
            self.test_pred, self.prompt_id)
        self.dump_predictions(self.dev_pred, self.test_pred, epoch)
        self.dev_prs, self.test_prs, \
            self.dev_spr, self.test_spr, \
            self.dev_tau, self.test_tau = \
            self.calc_correl(self.dev_pred, self.test_pred)
        self.dev_qwk, self.test_qwk, self.dev_lwk, self.test_lwk = \
            self.calc_qwk(self.dev_pred, self.test_pred)
        if self.dev_qwk > self.best_dev[0]:
            self.best_dev = [
                self.dev_qwk, self.dev_lwk, self.dev_prs, self.dev_spr,
                self.dev_tau
            ]
            self.best_test = [
                self.test_qwk, self.test_lwk, self.test_prs, self.test_spr,
                self.test_tau
            ]
            self.best_dev_epoch = epoch
            model.save_weights(
                self.out_dir + '/best_model_weights.h5', overwrite=True)
    ...
    def print_info(self):
        logger.info(
            '[D] loss: %.4f metric: %.4f mean: %.3f (%.3f) stdev: %.3f (%.3f)'
            % (self.dev_loss, self.dev_metric, self.dev_pred.mean(),
               self.dev_mean, self.dev_pred.std(), self.dev_std))
        logger.info(
            '[T] loss: %.4f metric: %.4f mean: %.3f (%.3f) stdev: %.3f (%.3f)'
            % (self.test_loss, self.test_metric, self.test_pred.mean(),
               self.test_mean, self.test_pred.std(), self.test_std))
        logger.info(
            '[D] QWK:  %.3f (Best @ %i: {{%.3f}}, %.3f, %.3f, %.3f, %.3f)' %
            (self.dev_qwk, self.best_dev_epoch, self.best_dev[0],
             self.best_dev[1], self.best_dev[2], self.best_dev[3],
             self.best_dev[4]))
        logger.info(
            '[T] QWK:  %.3f (Best @ %i: {{%.3f}}, %.3f, %.3f, %.3f, %.3f)' %
            (self.test_qwk, self.best_dev_epoch, self.best_test[0],
             self.best_test[1], self.best_test[2], self.best_test[3],
             self.best_test[4]))
    ...
    def print_final_info(self):
        ...
        logger.info(
            '  [DEV]  QWK: %.3f, LWK: %.3f, PRS: %.3f, SPR: %.3f, Tau: %.3f' %
            (self.best_dev[0], self.best_dev[1], self.best_dev[2],
             self.best_dev[3], self.best_dev[4]))
        logger.info(
            '  [TEST] QWK: %.3f, LWK: %.3f, PRS: %.3f, SPR: %.3f, Tau: %.3f' %
            (self.best_test[0], self.best_test[1], self.best_test[2],
             self.best_test[3], self.best_test[4]))
