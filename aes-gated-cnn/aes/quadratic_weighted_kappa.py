...
def quadratic_weighted_kappa(rater_a,
                             rater_b,
                             min_rating=None,
                             max_rating=None):
    '''
...
def liaesr_weighted_kappa(rater_a, rater_b, min_rating=None, max_rating=None):
    '''
    Calculates the liaesr weighted kappa
    liaesr_weighted_kappa calculates the liaesr weighted kappa
    value, which is a measure of inter-rater agreement between two raters
    that provide discrete numeric ratings.  Potential values range from -1
    (representing complete disagreement) to 1 (representing complete
    agreement).  A kappa value of 0 is expected if all agreement is due to
    ...
    liaesr_weighted_kappa(rater_a, rater_b), where rater_a and rater_b
    each correspond to a list of integer ratings.  These lists must have the
    same length.
    ...
    liaesr_weighted_kappa(X, min_rating, max_rating), where min_rating
    is the minimum possible rating, and max_rating is the maximum possible
    rating
    '''
    assert (len(rater_a) == len(rater_b))
...
def mean_quadratic_weighted_kappa(kappas, weights=None):
    ...
    weights = np.ones(
        np.shape(kappas)) if weights is None else weights / np.mean(weights)
...
def weighted_mean_quadratic_weighted_kappa(solution, submission):
    ...
    kappas = [
        quadratic_weighted_kappa(group[1]['essay_score'],
                                 group[1]['predicted_score'])
        for group in groups
    ]
